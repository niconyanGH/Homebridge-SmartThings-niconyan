...
/*
 * 이 클래스는 이 플러그인의 main constructor입니다.
...
export class HomebridgeSmartThings implements DynamicPlatformPlugin {
  ...
  constructor(
    ...
    /*
     * 이 이벤트는 발생하면 Homebridge가 캐시된 모든 액세서리를 디스크에서 복원했음을 의미합니다.
    ...
    */
    this.api.on('didFinishLaunching', () => {
      ...
      // 사용자가 보유한 디바이스를 찾고 Homebridge의 액세서리로 등록하는 method를 실행합니다.
      this.discoverDevices();
    });
  }

  /*
   * 이 function은 부팅 시 Homebridge가 캐시된 액세서리를 디스크에서 복원할 때 실행됩니다.
  ...
  async discoverDevices() {
    ...
    */
    ...
    for (const device of this.devices) {
      ...
      // 맥 주소 또는 시리얼번호 와 같은 디바이스의 고유한 정보로 UUID를 생성합니다.
